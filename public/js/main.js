...
function create() {
    ...
    for (var i = 0; i < 10; i++)
    {
        ...
        var bomb = bombs.create(game.world.randomX, game.world.randomY - game.world.height, 'bomb');
    ...
    //  The score
    statusText = game.add.text(16, 16, 'Collect stars and don\'t touch meteors', { fontSize: '32px', fill: '#FFF' });

    scoreText = game.add.text(16, 550, 'Score: 0', { fontSize: '32px', fill: '#FFF' });
    //  Our controls.
...
function update() {
    ...
    for(i=0; i<10; i++){
      if(bombs.children[i].position.y > 600){
          bombs.children[i].position.y = 0
          bombs.children[i].position.x = game.world.randomX
      }
    }
...
function collectBomb (player, bomb) {
    ...
    statusText.text = 'You Lost. Refresh page to start again';
